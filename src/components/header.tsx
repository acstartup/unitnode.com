        ...
        <header className="h-8 bg-white flex items-center px-8 sticky">
            ...
            <div className="flex-1 max-w-2xl" ref={searchRef}>
                <div className="relative">
                    ...
                    {/* Search Dropdown */}
                    {showSearchDropdown && filteredProperties.length > 0 && (
                        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                            {filteredProperties.map((property) => {
                                // Get tenant display based on tenants array or mainTenant
                                const getTenantDisplay = () => {
                                    if (property.tenants && property.tenants.length > 0) {
                                        // Find which tenant matches the search
                                        const searchLower = searchQuery.toLowerCase();
                                        const matchingTenant = property.tenants.find(t =>
                                            t.name.toLowerCase().includes(searchLower)
                                        );
                                        if (matchingTenant) {
                                            return `${matchingTenant.name} (${matchingTenant.relation})`;
                                        }
                                        // Default to first tenant if no match
                                        return `${property.tenants[0].name} (${property.tenants[0].relation})`;
                                    } else if (property.mainTenant && property.mainTenant !== 'N/A') {
                                        return `${property.mainTenant} (Main)`;
                                    }
                                    return null;
                                };

                                const tenantDisplay = getTenantDisplay();
                                const hasLease = property.tenants && property.tenants.length > 0 || (property.mainTenant && property.mainTenant !== 'N/A');
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {hasLease && tenantDisplay ? (
                                            <div className="text-xs text-gray-500">
                                                Owner: {property.ownerName || '-'} • Tenant: {tenantDisplay} • Rent: ${property.rent}
                                            </div>
                                        ) : (
                                            property.ownerName && (
                                                <div className="text-xs text-gray-500">
                    ...
                    {showSearchDropdown && searchQuery && filteredProperties.length === 0 && (
